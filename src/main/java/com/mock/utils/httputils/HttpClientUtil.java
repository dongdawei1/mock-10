package com.mock.utils.httputils;
...
public class HttpClientUtil {
	...
	public static void main(String[] args) {
		// HttpResult httpRequest= HttpClientUtil.sendPostJson(url, jsonData, null,
		// null, null);
	}
	/**
	 * 解析响应
	 */
	public static void checkResult(HttpResult httpRequest) {
		// HttpResult httpRequest= HttpClientUtil.sendPostJson(url, jsonData, null,
		// null, null);
	}
	...
	private static void init() {
		// 创建Connection配置对象
		connectionConfig = ConnectionConfig.custom().setMalformedInputAction(CodingErrorAction.IGNORE)
		...
		httpClient = HttpClients.custom().setDefaultConnectionConfig(connectionConfig)
				...
				.setDefaultCookieStore(cookieStore).setUserAgent(USER_AGENT).setMaxConnTotal(MAX_CONN_TOTAL)
	 ...
	 */
	public static HttpResult sendGet(String url, Map<String, String> parameters, Header[] headers, Cookie[] cookies,
			String charset) {
...
//       System.out.println(uri.toURL());
			...
			// 设置Header : Basic YWRtaW46RDhzblRob2NCWnkxWkNRMkx5
			httpGet.setHeader("Authorization", "Basic YWRtaW46RDhzblRob2NCWnkxWkNRMkx5");
			httpGet.setHeader("Accept",
					"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3");
			httpGet.setHeader("Accept-Language", "zh-CN,zh;q=0.9");
			httpGet.setHeader("Cache-Control", "max-age=0");
			httpGet.setHeader("Upgrade-Insecure-Requests", "1");
			httpGet.setHeader("Upgrade-Insecure-Requests", "10.169.121.154:8002");
//            
//            Cache-Control: 
...
//            		Upgrade-Insecure-Requests: 1
	 ...
	 * @param charset    请求数据的字符编码
	 */
	public static HttpResult sendPostForm(String url, Map<String, String> parameters, Header[] headers,
			Cookie[] cookies, String charset) {
		HttpResult httpResult = null;
		try {
			// 创建HttpPost
	...
	public static HttpResult sendPostJson(String url, String jsonData, Header[] headers, Cookie[] cookies,
		...
		try {
			...
			log.info("请求参数 :{}", httpPost.getEntity().toString());
			// 设置Header
			if (headers != null) {
				/**
				 * httpGet.setHeaders(headers)，重新设置Header，前面set或者add的Header都会被去掉
	...
	private static StringEntity assemblyStringEntity(String jsonData, String charset) {
		 ...
		 * Json类型的请求体，必须传一个不为null的StringEntity给服务端。 如果jsonData为null或""时，则进行特殊处理。
		 */
